...
for i=1:lx-1
    ...
    for k=2:lr-1
        ii=ii+1;
        A(ii,k+1)=-0.5*lambda*(1/r(k)+1/dr);
        A(ii,k-1)=-0.5*lambda*(1/dr);
        A(ii,k)=0.5*lambda*(1/r(k)+2/dr)+1;
        
        t(i,1)=t(i,2);
        B(ii)=t(i,k)+0.5*lambda*((1/r(k)+1/dr)*t(i,k+1)+(1/dr)*t(i,k-1)-(1/r(k)+2/dr)*t(i,k));
    end
    %--symmetry condition
    ii=ii+1;
    A(ii,1)=1;
