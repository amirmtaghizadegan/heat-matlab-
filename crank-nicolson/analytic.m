...
y=y0:dy:yl;
t=0:dt:Tl;
u=0:0.001:Tl;u(1)=eps;
T=zeros(lx,ly);
for i=1:lx
    for j=1:ly
        j21=(exp((y(j)./sqrt(4*a*u)+h*(sqrt(a*u))/k).^2).*erfc(y(j)./sqrt(4*a*u)+h*(sqrt(a*u))/k).*exp(-(x(i).^2+y(j).^2)./(4*a*u))./u.^(1.5));
        j21=trapz(u,j21);
        i5=(erfc(x(i)./sqrt(4*a*u)).*exp(-y(j)^2./(4*a.*u))./u.^(1.5));
        i5=0.5*trapz(u,i5);
        t=t(end);
