...
for n=1:lt-1
    ...
    for j=2:ly-1
        for i=2:lx-1
            ii=ii+1;
            % Calculation of A as constants matrix
            A(ii,id(i+1,j))=-lambda;
            A(ii,id(i,j+1))=-lambda;
            A(ii,id(i-1,j))=-lambda;
            A(ii,id(i,j-1))=-lambda;
            A(ii,id(i,j))=4*lambda+1;
            %Alocating initial and boundry conditions to t Matrix
            t(i,2,n)=(f0*dy/k)-(h*dy/k*t(i,1,n))+(t(i,1,n));
            t(:,:,1)=273;
            t(:,end,:)=273;
            t(1,:,:)=273;
            t(end,:,:)=273;
            % Alocating Matrix Answers to B
            B(ii)=t(i,j,n);
        end
    end
